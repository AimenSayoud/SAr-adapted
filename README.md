...
2. Ouvrir le notebook de la phase courante (`notebooks/<group>/phaseXX_*.ipynb`).
...
notebooks/                # un notebook mince par phase, groupé par question
  01_data/                # acquisition, réseau, masques, produits
  02_inversion/           # toutes les tentatives d'inversion par pixel (H1)
  03_corrections/         # atmosphère, géométrie de visée
  04_hypotheses/          # contraste de zones, agrégation, hydrologie (H2–H4)
  05_robustness/          # falsification, contrôles externes, référés
  06_manuscript/          # figures, tables, assemblage du document
...
<!-- PHASES:START -->
### 01_data — Data preparation — acquisition, network, masks, products
...
| [`phase03b`](notebooks/01_data/phase03b_pair_rates.ipynb) — Per-pair vertical rate, deramp and quality control | What vertical rate does each pair imply on its own? | ✅ | - |
| [`phase04`](notebooks/01_data/phase04_s2_fusion.ipynb) — Sentinel-2 fusion onto the HyP3 UTM grid | How do the optical observations align with the radar grid? | ✅ | §2.3, Table 3 |
| [`phase04b`](notebooks/01_data/phase04b_products.ipynb) — Ensemble, uncertainty and final products | What is the per-pixel uncertainty of the pairwise rates? | ✅ | - |
| [`phase05`](notebooks/01_data/phase05_water_mask.ipynb) — Dynamic water mask | Which pixels are inundated, and how often? | ✅ | §2.4, Fig. S2 |
| [`phase14`](notebooks/01_data/phase14_products.ipynb) — Uncertainties and final products | What are the final maps and their uncertainties? | ✅ | - |
| [`phase06`](notebooks/01_data/phase06_classification.ipynb) — Spatio-temporal classification, 5 classes | Do the pixels fall into distinct behavioural classes? | ✅ | - |
| [`phase07`](notebooks/01_data/phase07_quality_index.ipynb) — Quality index W_i | Can coherence and non-water fraction be combined into one weight? | ✅ | §3.1 |

### 02_inversion — Inversion — every attempt to recover per-pixel displacement (H1)

| Phase | Question | Status | Paper |
|---|---|---|---|
| [`phase15`](notebooks/02_inversion/phase15_annual_pairs.ipynb) — Annual "seasonal-annual" pairs | Does matching the seasonal state bypass temporal decorrelation? | ✅ | §4.1, Table 7 (estimator 3) |
| [`phaseA`](notebooks/02_inversion/phaseA_hybrid_insar_test.ipynb) — Hybrid network — the decisive InSAR test | Does combining short and long baselines recover usable pixels over the AOI? | ✅ | §4.1, Table 7 (estimator 4) |
| [`phaseC1`](notebooks/02_inversion/phaseC1_licsbas.ipynb) — LiCSBAS — an independent SBAS pipeline | Does a completely separate implementation close the loop? | 🔍 exploratory | - |
| [`phaseC2`](notebooks/02_inversion/phaseC2_phaselinking.ipynb) — Phase linking / DS-InSAR — first attempt | Does distributed-scatterer processing rescue the mat? | ⛔ superseded | superseded by phaseE2 |
| [`phaseE`](notebooks/02_inversion/phaseE_phaselinking_miaplpy.ipynb) — Phase linking with MiaplPy | Does the standard DS-InSAR toolchain run on these products? | ⛔ superseded | superseded by phaseE2 — MiaplPy needs an SLC archive we do not have |
| [`phaseE2`](notebooks/02_inversion/phaseE2_evd_phaselinking.ipynb) — Lightweight EVD phase linking — the direct test of H1 | Does the maximum-likelihood estimator, run on the delivered interferograms, recover the mat? | ✅ | §4.1.2, H1 — Table 8, Fig. 5, Fig. 6 |
| [`phase08`](notebooks/02_inversion/phase08_sbas_mintpy.ipynb) — SBAS inversion (MintPy) | Does a standard small-baseline inversion recover displacement? | ✅ | §4.1, Table 7 (estimator 1) |
| [`phase09`](notebooks/02_inversion/phase09_isbas.ipynb) — ISBAS inversion, with closure control | Does tolerating intermittent pixels rescue the inversion? | ✅ | §4.1, Table 7 (estimator 2) |
| [`phase10`](notebooks/02_inversion/phase10_comparison.ipynb) — SBAS versus ISBAS, quantitative | Do the two inversions disagree anywhere that matters? | ✅ | §4.1 |

### 03_corrections — Corrections — atmosphere and viewing geometry
...
| Phase | Question | Status | Paper |
|---|---|---|---|
| [`phase13`](notebooks/04_hypotheses/phase13_hydrology.ipynb) — Hydrological analysis and dielectric separation | Does the signal track water rather than motion? | ⛔ superseded | superseded by phaseI |
| [`phaseH`](notebooks/04_hypotheses/phaseH_predict_failure.ipynb) — Predicting where Sentinel-1 fails | Which covariates predict temporal coherence inside the mat? | ✅ | §4.2.5, Table 11, Fig. 10 |
| [`phaseD`](notebooks/04_hypotheses/phaseD_inside_vs_outside.ipynb) — Inside versus outside — is the floating mat a distinct target? | At matched land cover, does the mat decorrelate more than the same vegetation on stable ground? | ✅ | §4.2, H2 — Table 9, Fig. 7 |
| [`phaseDbis`](notebooks/04_hypotheses/phaseDbis_mechanism_spatial.ipynb) — Mechanism and spatial comparison — interior, edge, exterior, lake | Is the boundary sharp, and where exactly does coherence step? | ✅ | §4.2.3, Fig. 9 |
...
| Phase | Question | Status | Paper |
|---|---|---|---|
| [`phaseB`](notebooks/05_robustness/phaseB_egms_check.ipynb) — External check against EGMS | Does an independent ground-motion service see anything here? | 🔍 exploratory | - |
| [`phaseF`](notebooks/05_robustness/phaseF_nisar_lband.ipynb) — L-band test (NISAR) | Does the mat become coherent at 24 cm? | 🔍 exploratory | §5.4 — prospective, the archive does not cover 2022-2024 |
| [`phaseJ`](notebooks/05_robustness/phaseJ_falsification.ipynb) — Falsification — trying to demolish our own conclusion | Which of the eight alternative explanations survive? | ✅ | Appendix A — Table A1 |
| [`phaseK`](notebooks/05_robustness/phaseK_referee_response.ipynb) — Referee response | Do the referees' objections change any reported number? | ✅ | docs/paper/response_to_referees.md |
| [`phaseL`](notebooks/05_robustness/phaseL_gate.ipynb) — The gate, and the tests that matter either way | Do the nine round-2 tests hold? | ✅ | docs/paper/round2_report.md |

### 06_manuscript — Manuscript — figures, tables and the assembled document
