...
| **7** | `F07_paired_test.png` | Matched-cover paired test. (a) Distribution of coh(A) − coh(C) per interferogram; (b) scatter of A against C, points below the 1:1 line indicating lower mat coherence. |
| **8** | `F08_zone_distributions.png` | Per-zone distributions across five independent sensors: coherence, σ⁰ VV, RVI, Sentinel-2 wetness and temporal coherence. |
| **9** | `F09_radial_profiles.png` | Radial profiles of coherence, σ⁰ VV and RVI against signed distance to the peatland boundary. The step at distance zero marks a physical edge. |
...
| **16** | `F16_literature_context.png` | Our bound in context: raised-bog breathing, drained-fen subsidence, expected free flotation, and the value measured here. |
| **A1** | `FA1_causal_chain.png` | Proposed mechanism as a causal chain, from water table to the measured dielectric signal. |

## Supplementary figures

| # | File | Caption |
|---|---|---|
| **S1** | `S01_rgb_composite.png` | False-colour composite (R = σ⁰ VV, G = coherence, B = Sentinel-2 wetness) with zone outlines, and a zoom on the peatland. |
| **S2** | `S02_flooded_fraction.png` | Inundated-time fraction from the water mask, with zones A and B outlined. |
| **S3** | `S03_synthetic_validation.png` | Synthetic validation. On identical simulated data, per-pixel inversion returns −13.7 mm yr⁻¹ (36 % usable pixels) whereas aggregation returns −19.8 mm yr⁻¹ against a ground truth of −20. |
| **S4** | `S04_coherence_decay.png` | Coherence decay with temporal baseline, with fitted decorrelation times per zone. |
| **S5** | `S05_amplitude_dispersion.png` | Amplitude dispersion D_A: map and per-zone distributions against the 0.25 persistent-scatterer threshold. |
| **S6** | `S06_hydrology_freeze.png` | Coherence sensitivity to the water-table proxy, and coherence gain on freezing, by zone. |
